class Song {
  ...
  */
  ...
  async #setupSongDatabase(databaseQueryResult, requestType) {
    ...
    if (expired) {
      this.#songStreamingUrl = await this.#getStreamingUrl(this.#songYoutubeId)
  ...
  async #chooseCorrectSong(apiResponse) {
    ...
    for (let i = 0; i < apiResponse.length; i++) {
      ...
      */
      ...
      const resultArtistComparison = await compareArtists(spotifyArtists, song.artists, song.title)

      const foundArtists = resultArtistComparison.nrFound
      const artistNumberDeviation = Math.abs(resultArtistComparison.nrFound - spotifyArtists.length)
      const newTitle = resultArtistComparison.newTitle

      //compare titles (result#is deviation as Levenshtein distance)
      const normalTitleComparison = await compareTitles(this.#songTitle, song.title)

      const similarTitleComparison = await titleSimilarity(this.#songTitle, song.title)

      console.log(similarTitleComparison)

      const reducedTitleComparison = await compareTitles(this.#songTitle, newTitle)
      ...
      if (!bracketLess) {
        ...
        if (bracketLess.cleanText == bracketLess.extractedText) {
          bracketLessComparison = await compareTitles(this.#songTitle, bracketLess.cleanText)
      ...
      if (song.album != undefined && this.#songAlbum != undefined) {
        resultAlbumComparison = await albumTitleComparison(this.#songAlbum.name, song.album)
      }

      //compare duration (result is deviation in seconds)
      const timeDifference = compareSongDurations(this.#songDuration, song.duration.totalSeconds)

      //compare content rating (result is bool if matches)
      const resultContentRatingMatch = compareContentRatings(this.#songInfo.contentRating.label, song.isExplicit)
      ...
      if (resultTitleComparison == 0 && resultContentRatingMatch == true && artistNumberDeviation == 0) {
      ...
      let songDeviation =
        i * i + artistNumberDeviation * 6 + similarTitleComparison + resultTitleComparison * 1.5 + resultAlbumComparison * 1.3 + timeDifference * 0.5
      ...
      if (closestMatchDeviation == undefined || songDeviation < closestMatchDeviation) {
  ...
  async #getLikeStatusDb() {
    const response = await getFromDB(`SELECT * FROM playlistLikes WHERE id = '${this.#songSpotifyId}'`)
