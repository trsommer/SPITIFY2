...
async function chooseCorrectSong(apiResponse, info) {
  ...
  for (let i = 0; i < apiResponse.length; i++) {
    ...
    const normalTitleComparison = await compareTitles(info.name, song.title)

    const similarTitleComparison = await titleSimilarity(info.name, song.title)

    console.log(similarTitleComparison)

    const reducedTitleComparison = await compareTitles(info.name, newTitle)
    ...
    if (!bracketLess) {
      ...
      if (bracketLess.cleanText == bracketLess.extractedText) {
        bracketLessComparison = await compareTitles(info.name, bracketLess.cleanText)
    ...
    if (song.album != undefined && info.songAlbum != undefined) {
      resultAlbumComparison = await albumTitleComparison(info.album.name, song.album)
    }

    //compare duration (result is deviation in seconds)
    const totalSeconds = info.duration.totalMilliseconds / 1000

    const timeDifference = compareSongDurations(totalSeconds, song.duration.totalSeconds)

    //compare content rating (result is bool if matches)
    const resultContentRatingMatch = compareContentRatings(info.contentRating.label, song.isExplicit)
